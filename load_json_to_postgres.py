...
def parse_args():
    parser = argparse.ArgumentParser(description="Load JSON data into PostgreSQL with audit tracking.")
    parser.add_argument(
        "--json",
        default=os.path.join(os.path.dirname(__file__), "json", "bulk.json"),
...
def ensure_table_exists(conn, table_name):
    ...
    with conn.cursor() as cur:
        ...
        cur.execute(sql.SQL("ALTER TABLE {} ADD COLUMN IF NOT EXISTS loaddate TIMESTAMP NOT NULL DEFAULT now();").format(sql.Identifier(table_name)))
...
def main():
    ...
    with get_connection() as conn:
        ...
        try:
            file_size = os.path.getsize(args.json)
        except OSError:
            file_size = None
